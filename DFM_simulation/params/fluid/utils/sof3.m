function [krow, krog, Soco, Socr, Sorw, Sorg, Somax] = sof3(T, varargin)
% Construct oil-water and oil-gas relperm eval. functions from SOF3 table.
%
% SYNOPSIS:
%   [krow, krog, Soco, Socr, Sorw, Sorg, Somax] = sof3(table)
%
% PARAMETERS:
...
% $Revision: 10500 $
   ...
   Sorg  = zeros([1, ntab]);
   for k = 1 : ntab,
      [Soco(k), Socr(k), Sorw(k), Sorg(k), Somax(k)] = table_points(T{k});
...
function [Soco, Sorw, Sorg, Socr, Somax] = table_points(T)
   ...
   % 4) Critical water saturation (>= 0) is highest So for which both 
   %    krow(So)=0 and krog(So)=0.
   i    = find(T(:,2) > 0, 1, 'first');  assert (~isempty(i) && (i > 1));
   Socr = min(Sorw, Sorg);

   % 5) Maximal water saturation in table
   Somax = max(T(:,1));
end
