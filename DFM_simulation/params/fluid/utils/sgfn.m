function [krg, Sgcr, Sgmax, pc, pcinv] = sgfn(T, varargin)
% Construct gas relperm evaluation functions from SGFN table.
%
% SYNOPSIS:
%   [krg, Sgcr, Sgmax] = sgfn(table)
%
% PARAMETERS:
...
%
% SEE ALSO:
%  readRelPermTable, swfn, sgofn, swof, initBlackOilRelPerm.
...
function [Sgmax, Sgcr, Sgco] = table_points(T, Swco)
   ...
   assert (~(abs(T(1,1)) > 0));      % min(Sg) == 0.  %??
   assert (~(abs(T(1,2)) > 0));      % krg(min(Sg))  == 0.
