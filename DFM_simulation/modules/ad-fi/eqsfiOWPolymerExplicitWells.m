...
elseif ~opt.reverseMode
    [p, sW, c, qWs, qOs, wciPoly, pBHP]  = initVariablesADI(p, sW, c, qWs, qOs, wciPoly_num, pBHP);
else
...
eqs{3} =   (s.pv*(1-f.dps)/dt).*(pvMult.*bW.*sW.*c - pvMult0.*f.bW(p0).*sW0.*c0) ... 
         + (s.pv/dt).*(f.rhoR*((1-poro)./poro).*(pvMult.*f.ads(max(c, cmax))-pvMult0.*f.ads(cmax0))) ... % Adsorption isotherm effect
         + s.div(bWvP);
