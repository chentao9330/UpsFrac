function [V, P] = evalBasisFunc(faces, g, cg, BI, C, D, w, mob, varargin)
...
% $Revision: 10500 $
   ...
   [sub_map, ncells, nhf_blk] = mappings(g, cg, part, mob, opt);
   nBlk = max(part);
   ...
   if opt.Verbose,
      h = waitbar(0, 'Computing flux and pressure basis functions...');
      nFaces = numel(faces);
      fprintf('Computing flux and pressure basis functions... '), t0 = tic;
   end

   renum_v  = zeros([size(g.cells.faces,1), 1], 'uint32');
   iF_other = false([size(g.cells.faces,1), 1]          );
   renum_p  = zeros([g.cells.num        , 1], 'uint32');
   iG_other = false([g.cells.num,         1]          );
   ...
   for f = 1 : numel(faces),
      ...
      % coarse block.
      [p, press_ok(f)] = orth_press(p, theta(iG), part(iG), nBlk);

      % Reorder the unknowns/entries into [v,p] in order to have the
      % entries corresponding to blk(1) stored *first* in the following
      % SPARSE tuples.
      %
      % Algorithm:
      %   1) Create local numbering of entries (renum_* = ...)
      %   2) Mark entries outside blk(1) as being outside
      %      (*_other = true, *_other(blk(1)) = false)
      %   3) Put blk(1) entries first in index vector (ix_* = ...)
      %   4) Clean up blk(2:end) entries to prepare for next loop
      %      iteration, i.e., next basis function, (*_other = false).
      %
      % The formation process for the hybrid basis function matrices in
      % solveIncompFlowMS relies on being able to extract blk(1) entries
      % before blk(2) entries, and we can always store blk(1) entries
      % before any other entries, even when opt.Overlap > 0.  Moreover,
      % storing blk(1) entries before all other does not harm formation of
      % mixed basis function matrices.
      %
      renum_p(iG) = 1 : size(sG,1);              iG_other(iG)  = true ;
      renum_v(iF) = 1 : niF       ;              iF_other(iF)  = true ;
      iG1 = find(part == blk(1))  ;              iG_other(iG1) = false;
      iF1 = sub_map.hf(iG1)       ;              iF_other(iF1) = false;

      ix_p = [renum_p(iG1); renum_p(iG_other)];  iG_other(iG)  = false;
      ix_v = [renum_v(iF1); renum_v(iF_other)];  iF_other(iF)  = false;
      ...
      %
      V{f} = {iF(ix_v), v(ix_v), face, blk, ...
              nhf_blk(blk), avgmob(blk), opt.Overlap};
      P{f} = {iG(ix_p), p(ix_p), face, blk, ...
...
function [ops, nc, nhf] = mappings(g, cg, p, mob, opt)
