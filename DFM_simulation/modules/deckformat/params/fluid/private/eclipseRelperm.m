...
% $Revision: 10500 $
   ...
   assert (~ (family_I && family_II), ...
          ['Cannot mix keywords from famlies I and II. ', ...
   ...
   if isfield(deck.PROPS, 'SWFN'),
      ...
      [krw, Swco, pcow, Swcr, pcinv, Swmax] = ...
         swfn(deck.PROPS.SWFN{1});                                     %#ok
   ...
   if isfield(deck.PROPS, 'SGFN'),
      ...
      [krg, Sgcr, Sgmax, pcog, pcinv] = sgfn(deck.PROPS.SGFN{1});      %#ok
   ...
   if isfield(deck.PROPS, 'SOF2'),
      ...
      [kro, Soco, Socr, Somax] = sof2(deck.PROPS.SOF2{1});             %#ok
      kro = kro{1};
      if phase(V),
         krog = kro;
      elseif phase(A)
   ...
   if isfield(deck.PROPS, 'SOF3'),
      ...
      [krow, krog, Soco, Socr, Sorw, Sorg, Somax] = ...
   ...
   if all(phase),
      ...
      % --- STONE I -------------------
      if isfield(deck.PROPS, 'STONE1'),
         relperm = @(s, varargin) StoneI(s, Swco(1), krw, krow, krg, krog, ...
                                         krocw(1), Sowcr(1), Sogcr(1));
         relpermname = 'Stone I';

      % --- SIMPLE ------------------
      elseif any(isfield(deck.PROPS, {'SIMPLE'})),
         relperm =@(s, varargin) Simple(s, Swco(1), ...
                                         krw, krow, krg, krog, krocw(1));
         relpermname = 'Simple';

      % --- STONE II ------------------
      elseif any(isfield(deck.PROPS, {'STONE', 'STONE2'})),
         relperm =@(s, varargin) StoneII(s, Swco(1), ...
                                         krw, krow, krg, krog, krocw(1));
         relpermname = 'Stone II';

      % --- VERTICAL AVERAGE IN CELL --
      else
         relperm = @(s, varargin) threephase(s, Swco(1), ...
                                             krw, krow, krg, krog);
         relpermname = 'ECLIPSE default';
      end

      pcap    = @(s) threephasecap(s, pcow, @null, pcog);


   % ----------------------------------------------------------------------
   % Water-oil model
   % ----------------------------------------------------------------------
   elseif phase(A) && phase(L),
      if(family_II)
         krow=kro;
      end
      relperm = @(s, varargin) twophase(s, @(s) krw (s(:, pos(A))), ...
                                           @(s) krow(s(:, pos(L))));


      pcap    = @(s) twophasecap(s(:, pos([A, L])), pcow, @null);
   ...
   elseif phase(L) && phase(V),
      relperm = @(s, varargin) twophase(s, @(s) krog(s(:, pos(L))), ...
                                 @(s) krg (s(:, pos(V))));
      pcap    = @(s) twophasecap(s(:, pos([L, V])), @null, pcog);
...
function varargout = StoneI(s, Swco, krw, krow, krg, krog, krocw, ...
                            Sowcr, Sogcr)
...
function varargout = Simple(s, Swco, krw, krow, krg, krog, krocw)
% This function implements the second method to compute oil relative
% permeabilities suggested by Stone.
%
% The oil relatve permeability kro is given by
