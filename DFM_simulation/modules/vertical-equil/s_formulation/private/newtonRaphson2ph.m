...
% $Revision: 9020 $
   ...
   while t < tf && dt > mints,
      ...
      while redo_newton,
         ...
         sn = resSol; sn.s(:) = 0.5;
         if isfield(sn, 's_max'),
               sn.s_max = max(sn.s(:,1), opt.resSol_old.s_max);
         end
         %-----------------------------------------------------------------
...
%            end
      ...
      if isfield(resSol, 's_max'),
         % Save minimum saturation for use in modeling of relative
         % permeability hysteresis.
         resSol.s_max = max(resSol.s(:,1), resSol.s_max);
      end    
