function [G, Gt, rock, rock2D, bcIxVE] = makeIGEMSmodel(usemex)
...
try
   disp(' -> Reading IGEMS.mat');
   d = fileparts(mfilename('fullpath'));
   load(fullfile(d,'IGEMS'));
   return;
catch me
...
try
   grdecl = readGRDECL(fullfile(ROOTDIR, 'examples', 'data', 'igems', ...
                                'IGEMS.GRDECL'));
catch me
  disp('Download data from:')
  disp(['http://www.sintef.no/Projectweb/MRST/', ...
        'Downloadable-Resources/Download-IGEMS-Data-Set/'])
  disp(['Put data in ', fullfile(ROOTDIR, 'examples', 'data', 'igems')]);
end
...
%% Store data
disp(' -> Writing IGEMS.mat');
save(fullfile(d,'IGEMS'), 'G', 'Gt', 'rock', 'rock2D', 'bcIxVE');
end
