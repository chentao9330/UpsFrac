function plotPanelVE(G, Gt, W, sol, t, vol, varargin)
...
%   t       - Time
%   vol     - Vector of (free, trapped, total) volume
%   opt     - Various options
...
subplot(2,3,3), cla
str1 = ['trapped ' num2str(round(vol(2)/vol(3)*100)) ' %'];
str2 = ['free ' num2str(round(vol(1)/vol(3)*100)) ' %'];
pie([max(vol(2),eps) max(vol(1),eps)],{str1, str2});
title(['Total volume: ', ...
...
axis tight off, view([0 0]), title('CO2-saturation, x-slice (cyan)');
caxis(prm.Saxis);

subplot(4,3,11:12); cla % yslice
plotCellData(G_y, sol.s(cells_sub_y),'EdgeColor','w','EdgeAlpha',.1);
axis tight off, view([90 0]), title('CO2-saturation, y-slice (magenta)');
caxis(prm.Saxis);
drawnow
