function state = solveMSFV_TPFA_Incomp(state, G, CG, T, fluid, varargin)
...
% $Revision: 10500 $
    ...
    if strcmp(opt.Discretization, 'tpfa')
        s = incompTPFA(state, G, T, fluid, 'wells', opt.wells, 'bc', opt.bc, 'Verbose', false, 'MatrixOutput', true, 'LinSolve', @(A,b) zeros(size(b)) );
    else
    ...
    if opt.Iterations
        ...
        Nn = CG.cells.num;
        % R: Sparse equivialent of R = [zeros(Nn,Nf-Nn) speye(Nn)];
...
%                 U = gmres(@(u) A*u,r,opt.Restart,10e-9,opt.Iterations,invG, [], U);
                ...
                [U, flag, relres, iter, resvec] = gmres(rr, r ,opt.Restart,opt.Tolerance,opt.Iterations,invG, [], U);
                state.residuals = resvec;
...
%                 U = gmres(@(u) A*u,r,opt.Restart,10e-9,opt.Iterations,invG, [], U);
                [U, flag, relres, iter, resvec] = gmres(@(u) A*u,r,opt.Restart,opt.Tolerance,opt.Iterations,[], [], U);
    ...
    if opt.Reconstruct
        ...
        sp = DG.P_flux'*mldivide(D, DG.P_flux*state.rhs(1:Nf) - (Abar - D)*DG.P_flux*state.pressure);
        p(1:numel(sp)) = sp;
        state.pressure_reconstructed = sp(1:G.cells.num);
