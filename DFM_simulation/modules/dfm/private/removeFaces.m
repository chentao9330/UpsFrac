...
% $Revision: 6992 $
   ...
   [G.cells.faces,G.cells.facePos] = removeFromPackedData(G.cells.facePos,...
                                                        G.cells.faces, f);
   G.cells.faces(:,1) = facemap(G.cells.faces(:,1));
   G.cells.numFaces = int8(diff(G.cells.facePos));
   ...
   if isfield(G.faces, 'tag'), G.faces.tag      (ind,:) = []; end
