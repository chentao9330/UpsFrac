...
% $Revision: 10500 $
   ...
   [F, fcontrib_dp, wcontrib_dp, gflux] = ...
      assemble_residual(luAc, Af, mob, state, state0,  G, trans, ...
                        dt, pv, density, opt, OP, save.luAc);
...
function dp = ...
   ...
   while ~(norm_F <   norm_F0),
      ...
      F = assemble_residual(luAc, Af, mob, state, state0, G, ...
                            trans, dt, pv, density, opt, OP, luAc);
...
function [state, ok] = update_masses_flux(z0, flux, Af, conmob, dt, pv, OP)
   ...
   f            = bsxfun(@rdivide, conmob, sum(conmob,2));
   comp_conflux = mmultiply(Af, bsxfun(@times, flux, f));
   ...
   if ~ok,
      ...
      warning(['Mass update produces negative ', ...
