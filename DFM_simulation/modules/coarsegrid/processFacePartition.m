...
% $Revision: 10500 $
   ...
   if g.griddim == 2,
      ...
      clear tmp fno
   elseif g.griddim ==3,
      % Create edge -> face map for faces in f ----------------------------
      % For each edge ei connected to ne of the faces in f, find all faces
      % in f also connected to ei. 
      assert(false, ...
         'Support for 3D not implemented in processFacePartition');
   else
      error('Huh!? Missing field ''griddim'' in grid...');
   end
