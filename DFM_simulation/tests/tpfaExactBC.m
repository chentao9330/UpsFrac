...
solver = @(bc) incompTPFA(state, G, T, fluid, 'bc', bc);
...
n(3) = norm(xr.flux-flux, inf)/norm(flux, inf);

fprintf('Max error in pressure, face pressure and face flux %d [Z]\n', max(n));
...
fprintf('Max error in pressure, face pressure and face flux %d [X]\n', max(n));
if norm(n, inf) > 1, error('Wrong answer'); end
