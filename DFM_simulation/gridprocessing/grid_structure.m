...
%
%       - griddim -- The dimension of the grid which in most cases will
%                    equal size(G.nodes.coords,2).
%
...
%                    information of cell 'i' is found in the submatrix
%
...
%
%    - faces     --  A (G.cells.facePos(end)-1)-by-2 array of global faces
%                    connected to a given cell.  Specifically, if
%                    cells.faces(i,1)==j, then global face cells.faces(i,2)
%                    is connected to global cell `j'.
%
%                    To conserve memory, only the second column is actually
%                    stored in the grid structure.  The first column may be
%                    re-constructed using the statement
%
%                              rldecode(1 : G.cells.num, ...
%                                       diff(G.cells.facePos), 2) .'
%
%                    A grid constructor may, optionally, append a third
%                    colum to this array.  In this case cells.faces(i,3)
%                    often contains a tag by which the cardinal direction
%                    of face cells.faces(i,2) within cell cells.faces(i,1)
%                    may be distinguished.  Some ancillary utilities within
%                    the toolbox depend on this specific semantics of
%                    cells.faces(i,3), e.g., to easily specify boundary
%                    conditions (functions 'pside' and 'fluxside').
%
...
%                       ijk        = [ijk{:}];
%
%                    In the latter case, ijk(i,:) is the global (I,J,K)
%                    index of cell 'i'.
%
...
%
%    - nodes     --  A (G.faces.nodePos(end)-1)-by-2 array of vertices in
%                    the grid.  Specifically, if faces.nodes(i,1)==j, then
%                    local vertex `i' is part of global face number `j' and
%                    corresponds to global vertex faces.nodes(i,2).  To
%                    conserve memory, only the last column is stored.  The
%                    first column can be constructed using the statement
%
%                              rldecode(1 : G.faces.num, ...
%                                       diff(G.faces.nodePos), 2) .'
%
%    - neighbors --  A G.faces.num-by-2 array of neighbouring information.
%                    Global face `i' is shared by global cells
%                    neighbors(i,1) and neighbors(i,2).  One of
%                    neighbors(i,1) or neighbors(i,2), but not both, may be
%                    zero, meaning that face `i' is an external face shared
%                    only by a single cell (the non-zero one).
%
%    - tag       --  A G.faces.num-by-1 array of face tags.  A tag is a
%                    scalar.  The exact semantics of this field is
%                    currently undecided and subject to change in future
%                    relases of MRST.
%
%    - areas     --  A G.faces.num-by-1 array of face areas.
%
%    - normals   --  A G.faces.num-by-d array of *AREA WEIGHTED*, directed
%                    face normals in R^d.  The normal on face `i' points
%                    from cell G.faces.neighbors(i,1) to cell
%                    G.faces.neighbors(i,2).
%
