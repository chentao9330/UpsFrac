...
%
% SEE ALSO:
%   readGRDECL, deactivateZeroPoro, removeCells.
...
if isfield(grdecl, 'ACTNUM'),
   a = reshape(grdecl.ACTNUM==1, grdecl.cartDims);
else
...
assert(~any(dz(:) < 0), ...
   'ZCORN must be strictly non-decreasing along each pillar.');
clear a z dz
...
%%  -----------------------------------------------------------------------
% Add auxillary top and bottom layer to ensure correct processing of outer
% boundary at faults.
...
%%  -------------------------------------------------------------------------
% Find unique points in input.  Unique sorts with most significant number
% in first column. We must use column order Z, Y, X to obtain natural
% ordering of points.
...
% In all caluculations we pass arrays of point numbers P, block numbers B
% and the point coordinates (G.nodes.coords)

P = reshape(from, G.cartDims*2); clear from to
...
dispif(opt.Verbose, 'Processing regular i-faces\n'); ticif(opt.Verbose),
...
dispif(opt.Verbose, 'Processing i-faces on faults\n'); ticif(opt.Verbose)
...
dispif(opt.Verbose, 'Processing regular j-faces\n'); ticif(opt.Verbose),
...
dispif(opt.Verbose, 'Processing j-faces on faults\n'); ticif(opt.Verbose),
G = findFaults(G, P, B, actnum, tags, opt);
tocif(opt.Verbose),
dispif(opt.Verbose, '\n');
...
dispif(opt.Verbose, 'removing %d artifical cells at top/bottom\n', ...
       numAuxillaryCells);
...
if opt.SplitDisconnected,
   G = splitDisconnectedGrid(G, opt.Verbose);
end
...
if all(all (pa == pb))
   ...
   Corners    = reshape(pa', [], 1);
    ...
    %   o---  A
    %   *---  B
    %      x  intersection between pot and bottom envelope
    %
    %
