...
% $Revision: 10500 $
   ...
   % gravity on rhs.
   %no_grav = ~(norm(g_vec(1 : size(G.nodes.coords,2))) > 0 || isfield(G, 'grav_pressure'));
   no_grav = ~(norm(g_vec(1:G.griddim)) > 0 || isfield(G, 'grav_pressure'));
   ...
   totmob = sum(mob, 2);
   if(opt.use_trans)
     assert(numel(T)==G.faces.num);
     fmob = accumarray(cf, totmob(cellNo), [nf, 1]);
     fmob = fmob./sum(G.faces.neighbors>0,2);
     ft= T.*fmob;
     % fake half face trans
     th=ft.*sum(G.faces.neighbors>0,2);
